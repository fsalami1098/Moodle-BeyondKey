...
function theme_mb2nl_user_bookmarks()
{
	...
	get_string('togglemenuitem', 'theme_mb2nl', array('menuitem' => get_string('mybookmarks', 'theme_mb2nl'))) . '"></button>';
	...
	foreach ( $tempbookmarks as $b )
	{
		...
		if ( $b_arr0 )
		{
			$output .= '<li class="level-2" data-url="' . $b_arr0 . '">';
			...
			$output .= '<button type="button" class="theme-bookmarks-form bookmark-edit themereset" data-url="' . $b_arr0 . '" data-mb2bktitle="' . trim($title) . '"' . $bookmark_manage_attr . ' aria-label="' . get_string('edit') . '"><i class="fa fa-pencil"></i></button>';
			$output .= '<button type="button" class="theme-bookmarks-form bookmark-delete themereset" data-url="' . $b_arr0 . '" data-mb2bktitle="' . trim($title) . '" aria-label="' . get_string('remove') . '"><i class="fa fa-times"></i></button>';
			$output .= '</div>';
			$output .= '</li>';
	...
	$output .= '<button type="button" class="theme-bookmarks-form' . $bookmark_addremove . ' themereset mb2mm-action" data-url="' . trim($bookmarkurl) . '" data-mb2bktitle="' . theme_mb2nl_page_title() . '"' . $bookmark_manage_attr . '>';
...
function theme_mb2nl_user_bookmarks_modal ()
{
	...
	$createurl = new moodle_url(theme_mb2nl_themedir() . '/mb2nl/lib/lib_ajax_bookmarks_manage.php', array());
	$bookmarkurl = htmlspecialchars_decode(str_replace($CFG->wwwroot,'',$PAGE->url));

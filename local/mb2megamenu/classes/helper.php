...
if ( ! class_exists( 'Mb2megamenuHelper' ) )
{
    class Mb2megamenuHelper
    {
        ...
        public static function menu_template( $id, $pos )
        {
            ...
            $langmenu = self::get_language_menu();  // Menu ID

            if ( $id )
            {
                // Menu by ID
                $menu = self::get_available_menus( $id );
            }
            elseif ( $langmenu )
            {
                // Menu by language
                $menu = self::get_available_menus( $langmenu );
            }
            elseif ( $firstmenu )
            {
                // Set the first available menu
                $menu = self::get_available_menus( $firstmenu );
            } 
            ...
            $id = self::menu_attrid($menu);
            $items = json_decode($menu->menuitems);
            ...
            $output .= '<div id="main-navigation" class="' . $cls . '" style="' . self::menu_style($menu) . $themestyle . '">';
            ...
            $output .= '<!-- Mb2 Mega Menu --><ul id="' . $id . '" class="mb2mm' . self::menu_cls($menu) . '">';  
        ...
        {            
            ...
            $wrapcls .= ' cols-' . self::get_setting($item, 'columns');
            ...
            $output .= self::get_setting($item, 'sublabel') ? '<span class="mb2mm-sublabel">' . $item->settings->sublabel . '</span>' : '';            
            $output .= '</span>'; // mb2mm-item-content

            // Highlight label
            $output .= self::get_setting($item, 'hlabel') ? '<span class="mb2mm-hlabel" aria-hidden="true">' . self::get_setting($item, 'hlabel') . '</span>' : '';
            ...
            $output .= $ischild ? '<button type="button" class="mb2mm-toggle themereset" aria-label="' . get_string('togglemenu', 'local_mb2megamenu', $itemlabel) . '"></button>' : '';
        ...
        /*
         *
         * Method to get first language menu
         *
         */
        public static function get_language_menu()
        {
            ...
            {
                $languages = json_decode($menu->language);
         ...
         *
         */
        public static function get_available_menus($id = 0)
        {
            global $DB;
         ...
         *
         */
        public static function menu_style($menu)
        {
         ...
         *
         */
        public static function menu_cls($menu)
        {
         ...
         *
         */
        public static function menu_attrid( $menu )
        {
            
            return 'mb2mm_' . $menu->id;                                
